...
def load_data():
    ...
    df['status']      = df['pct_sold'].apply(
        lambda x: ' Healthy' if x >= 95 else (' At Risk' if x < 70 else ' Moderate')
    )
    return df
...
def get_recommendations(project_row, df):
    ...
    # - ISSUE 1: HIGH PRICE -
    if price > 9500:
        issues.append(f" Price Rs.{price:,}/sqft is above the Rs.9,500 market threshold")
        rec_price = round(avg_healthy_price * 0.95)
        recs.append({
            'icon': '', 'priority': 'HIGH',
            'title': f'Reduce launch price to Rs.{rec_price:,}/sqft',
            'detail': f'Healthy projects in Bagaluru average Rs.{int(avg_healthy_price):,}/sqft. '
                      f'A price cut to Rs.{rec_price:,} (-{((price-rec_price)/price*100):.0f}%) '
                      f'could unlock buyers in the Rs.7,000-9,000 sweet spot. '
                      f'Learn from Brigade El Dorado (100% sold at Rs.8,600-9,345/sqft).',
    ...
    if project_row['developer_tier'] in ['New', 'Small', 'Luxury/New'] and pct < 80:
        ...
        recs.append({
            ...
            'detail': 'NVG and MJR face brand trust deficit. A CRISIL developer rating costs '
                      'Rs.3-5L but unlocks access to SBI/HDFC approved project lists, reaching '
...
if page == " Market Overview":
    ...
    with c4:
        st.markdown(f'<div class="metric-card amber"><div class="metric-val">{sigma:.1f}sigma</div><div class="metric-label">Sigma Level (Target: 6sigma)</div></div>', unsafe_allow_html=True)
    with c5:
    ...
    fig3 = px.scatter(
        ...
        color_discrete_map={' Healthy':'#16A34A',' At Risk':'#E24B4A',' Moderate':'#EF9F27'},
        hover_name='project',
        hover_data={'developer':True,'total_units':True,'unsold_units':True,'delay_months':True},
        labels={'price_sqft':'Launch Price (Rs./sqft)','pct_sold':'% Sold','status':'Status'}
    )
    fig3.add_hline(y=70, line_dash="dash", line_color="orange", annotation_text="70% threshold")
    fig3.add_vline(x=9500, line_dash="dash", line_color="red", annotation_text="Rs.9,500 danger zone")
...
# -----------------------------------------------------------------
    ...
    if selected_dev == " All Builders (Market Overview)":
        ...
        st.dataframe(
            dev_projects[display_cols].rename(columns={
                ...
                lambda v: 'background-color:#dcfce7' if '' in str(v) else
                          ('background-color:#fee2e2' if '' in str(v) else
        ...
        if len(at_risk) > 0:
            st.markdown(f"###  At-Risk Projects ({len(at_risk)} below 70% sold)")
            for _, row in at_risk.iterrows():
                with st.expander(f" {row['project']} - {row['pct_sold']:.1f}% sold | {row['unsold_units']} units unsold"):
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Price/sqft", f"Rs.{row['price_sqft']:,}")
...
# -----------------------------------------------------------------
    ...
    sel_proj = st.selectbox(" Select Project to Simulate", proj_options)
    ...
    with col1:
        ...
        - Price: **Rs.{proj_row['price_sqft']:,}/sqft**
    ...
    if st.button(" Run Digital Twin Simulation", type="primary", use_container_width=True):
        ...
        if apply_intervention:
            ...
            s3.metric("Recovery Gain", f"+{int(final_sold_intv - final_sold_base):,} units",
                      f"Rs.{int((final_sold_intv - final_sold_base) * proj_row['price_sqft'] * 1000 / 1e7):.1f} Cr revenue")

        # Simulation log
        st.markdown("###  SimPy Event Log (First 10 Months)")
...
# -----------------------------------------------------------------
    ...
    if selected_dev == " All Builders (Market Overview)":
        ...
        st.markdown(f"###  All At-Risk Projects ({len(at_risk_all)} below 70% sold)")
        ...
        if len(at_risk) == 0:
            ...
            if len(healthy_dev) > 0:
                ...
                for _, row in healthy_dev.iterrows():
                    ...
                    Price: Rs.{row['price_sqft']:,}/sqft . Size: {row['unit_size_min']}-{row['unit_size_min']} sqft . 
            ...
            with col1:
                ...
                . Price: Rs.{healthy_all['price_sqft'].mean():,.0f}/sqft (range: Rs.{healthy_all['price_sqft'].min():,}-Rs.{healthy_all['price_sqft'].max():,})<br>
            ...
            with col2:
                ...
                . Price: Rs.{at_risk['price_sqft'].mean():,.0f}/sqft (often >Rs.9,500)<br>
...
# -----------------------------------------------------------------
    ...
    st.markdown("###  Configure Baseline Period")
    ...
    # Moving Range sigma estimate (d2=1.128 for n=2 subgroups - standard I-MR chart)
    ...
    k2.metric("UCL  (+3sigma MR)",             f"{ucl:.0f}",       "units/month")
    k3.metric("LCL  (-3sigma MR)",             f"{lcl:.0f}",       "units/month")
    ...
    if ooc.sum() > 0:
        ...
        for i in range(len(MONTHS)):
            if ooc[i]:
                direction = "(up) Above UCL" if ooc_above[i] else "(down) Below LCL"
        ...
        st.dataframe(
            ooc_df.style.applymap(
                lambda v: 'background-color:#fee2e2;color:#991b1b' if '(up)' in str(v)
                     else ('background-color:#fef3c7;color:#92400e' if '(down)' in str(v) else ''),
                subset=['Direction']
            ),
            use_container_width=True
        )
    ...
    with st.expander(" Why was the old SPC showing 0 out-of-control points? (Bug explained)"):
        ...
        **Root Cause of the Bug:**
        ...
        This inflated the standard deviation to sigma = 203, making UCL = 820.
        ...
        **The Fix:**
        ...
        - Method = I-MR chart (Moving Range sigma estimate using d2=1.128) - the proper standard
